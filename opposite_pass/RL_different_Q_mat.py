...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct 11 11:26:02 2018
...
selfish_Q_p1 = np.zeros((len(mapList[0]),len(mapList),len(mapList[0]),len(mapList),NUM_ACTIONS,NUM_ACTIONS))
selfish_Q_p2 = np.zeros((len(mapList[0]),len(mapList),len(mapList[0]),len(mapList),NUM_ACTIONS,NUM_ACTIONS))
...
root = '/home/sleek_eagle/research/gameTheory/E_learning/'
...
#***********************
#calculate empathy matrics when both agents can see each other's reward matrices
#*********************************
def get_p1_reward(state):
  p1_rewards = selfish_Q_p1[state[0]][state[1]][state[2]][state[3]]
  return p1_rewards

def get_p2_reward(state):
  flipped = flip_state(state)
  p2_reward = selfish_Q_p2[flipped[0]][flipped[1]][flipped[2]][flipped[3]]
  return p2_reward
...
#***********************
#calculate empathy matrics when bothe agents cannot see each other's reward matrices
#each of them have to infer each other's rewards from their own
#*********************************
def get_rewards_p1(state):
  p1_rewards = selfish_Q_p1[state[0]][state[1]][state[2]][state[3]]
  flipped = flip_state(state)
  p2_rewards = selfish_Q_p1[flipped[0]][flipped[1]][flipped[2]][flipped[3]]
  return p1_rewards,p2_rewards

def get_rewards_p2(state):
  p1_rewards = selfish_Q_p2[state[0]][state[1]][state[2]][state[3]]
  flipped = flip_state(state)
  p2_rewards = selfish_Q_p2[flipped[0]][flipped[1]][flipped[2]][flipped[3]]
  return p1_rewards,p2_rewards

def get_empathy_matrix_p1(state,e):
  p1_reward,p2_reward = get_rewards_p1(state)
  emp_matrix = np.zeros((NUM_ACTIONS,NUM_ACTIONS))
  for i in range(0,NUM_ACTIONS):
    for j in range(0,NUM_ACTIONS):
      emp_matrix[i][j] = (1-e)*p1_reward[i][j] + e*p2_reward[flip_action(j)][flip_action(i)]
...
def get_empathy_matrix_p2(state,e):
  p1_reward,p2_reward = get_rewards_p2(state)
  emp_matrix = np.zeros((NUM_ACTIONS,NUM_ACTIONS))
  for i in range(0,NUM_ACTIONS):
    for j in range(0,NUM_ACTIONS):
      emp_matrix[i][j] = (1-e)*p1_reward[i][j] + e*p2_reward[flip_action(j)][flip_action(i)]
...
#when each player cannot see the other's reward matrix. They have to infer this based on 
#their own matrix
def get_max_actions_oblivious(state):
  p1_empathy_matrix = get_empathy_matrix_p1(state,E[0])
  p2_empathy_matrix = get_empathy_matrix_p2(state,(1-E[1]))
  p1_max = get_max_row_col(p1_empathy_matrix)[0]
  p2_max = get_max_row_col(p2_empathy_matrix)[1]
  return [p1_max,p2_max],[p1_empathy_matrix[p1_max][p2_max],p2_empathy_matrix[p1_max][p2_max]]
...
def select_actions(state,prob,e1,e2,oblivous):
  prob *=100
  r = [random.randint(0,100),random.randint(0,100)]
  if (oblivous):
    max_actions = get_max_actions_oblivious(state)[0]  
  else:
...
def get_state_values(state,oblivous):
  if (oblivous):
    [p1_action,p2_action],[p1_max,p2_max] = get_max_actions_oblivious(state)
  else:
    [p1_action,p2_action],[p1_max,p2_max] = get_max_actions(state)
  return p1_max,p2_max
 
def Q_update_p1(state,actions,reward,next_state_value):
  global selfish_Q_p1
  selfish_Q_p1[state[0]][state[1]][state[2]][state[3]][actions[0]][actions[1]] = (1-ALPHA)*selfish_Q_p1[state[0]][state[1]][state[2]][state[3]][actions[0]][actions[1]] + ALPHA*(reward+GAMMA*next_state_value)

def Q_update_p2(state,actions,reward,next_state_value):
  global selfish_Q_p2
  selfish_Q_p2[state[0]][state[1]][state[2]][state[3]][actions[0]][actions[1]] = (1-ALPHA)*selfish_Q_p2[state[0]][state[1]][state[2]][state[3]][actions[0]][actions[1]] + ALPHA*(reward+GAMMA*next_state_value)

def update_Q_values(state,actions,next_state,rewards,oblivious):
  ...
  next_state_values = get_state_values(next_state,oblivious)
  
  Q_update_p1(p1_state,actions,rewards[0],next_state_values[0])
  
  Q_update_p2(p2_state_action[0:4],p2_state_action[4:6],rewards[1],next_state_values[1])

def get_avg_Q_value():
  return np.mean(selfish_Q_p1),np.mean(selfish_Q_p2)
def get_diff_Q():
  return np.mean(np.absolute(selfish_Q_p1 - selfish_Q_p2))
...
def choose_random_E():
  global E
  E = [random.randint(0,10)/10,random.randint(0,10)/10]
...
l1 = []
l2 = []
def run_episode(explore,delay,oblivious,random_empathy):
  if(random_empathy):
    choose_random_E()
  init()
  itr = 0
  while(1):
    ...
    itr +=1
    state = get_state()
    actions = select_actions(state,explore,E[0],E[1],oblivious)
    ...
    update_Q_values(state,actions,next_state,rewards,oblivious)
    update()
    avgs = get_avg_Q_value()
    l1.append(avgs[0])
    l2.append(avgs[1])
...
while(1):
  run_episode(0,0.4,oblivious,random_empathy)
  diff.append(get_diff_Q())
